...
func TestLowerRejectsUnsupportedStatementBeforeIR(t *testing.T) {
	...
	_, err = Lower(program)
	if err == nil || !strings.Contains(err.Error(), "native subset") || !strings.Contains(err.Error(), "EmptyStatement") || !strings.Contains(err.Error(), "at offset 0") || !strings.Contains(err.Error(), entry) {
