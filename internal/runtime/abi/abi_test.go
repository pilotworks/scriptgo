...
int scriptgo_array_get_unknown(void *, double, void *);
int scriptgo_array_length(void *, int64_t *);
int scriptgo_array_release(void *);
int scriptgo_object_new(int64_t, void **);
int scriptgo_object_type_set(void *, const char *);
int scriptgo_object_number_set(void *, int64_t, double);
int scriptgo_object_number_get(void *, int64_t, double *);
int scriptgo_object_property_unknown_set(void *, const char *, const scriptgo_value *);
int scriptgo_object_property_unknown_get(void *, const char *, scriptgo_value *);
...
int main(void) {
    ...
    void *boxed_array = NULL;
    if (scriptgo_array_new(2, sizeof(scriptgo_value), &boxed_array) != 0) return 14;
    scriptgo_value boxed_element = {SCRIPTGO_TAG_NUMBER, 0, 0, 0};
    double boxed_value = 9;
    memcpy(&boxed_element.payload, &boxed_value, sizeof(boxed_value));
    if (scriptgo_array_set(boxed_array, 0, &boxed_element) != 0) return 15;
    boxed_element.tag = SCRIPTGO_TAG_BOOLEAN;
    boxed_element.payload = 1;
    if (scriptgo_array_set(boxed_array, 1, &boxed_element) != 0) return 16;
    scriptgo_value boxed_element_read;
    scriptgo_value_init_undefined(&boxed_element_read);
    if (scriptgo_array_get_unknown(boxed_array, 0, &boxed_element_read) != 0 ||
        boxed_element_read.tag != SCRIPTGO_TAG_NUMBER) return 17;
    scriptgo_value_release(&boxed_element_read);
    if (scriptgo_array_get_unknown(boxed_array, 1, &boxed_element_read) != 0 ||
        boxed_element_read.tag != SCRIPTGO_TAG_BOOLEAN || boxed_element_read.payload != 1) return 18;
    scriptgo_value_release(&boxed_element_read);
    if (scriptgo_array_release(boxed_array) != 0) return 19;
    ...
    if (scriptgo_object_type_set(object, "__json__") != 0) return 17;
    scriptgo_value boxed_number = {SCRIPTGO_TAG_NUMBER, 0, 0, 0};
    memcpy(&boxed_number.payload, &number, sizeof(number));
    if (scriptgo_object_property_unknown_set(object, "value", &boxed_number) != 0) return 18;
    scriptgo_value boxed_read;
    scriptgo_value_init_undefined(&boxed_read);
    if (scriptgo_object_property_unknown_get(object, "value", &boxed_read) != 0 ||
        boxed_read.tag != SCRIPTGO_TAG_NUMBER || boxed_read.payload != boxed_number.payload) return 19;
    scriptgo_value_release(&boxed_read);
