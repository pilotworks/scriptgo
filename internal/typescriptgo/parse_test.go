...
func TestCheckExposesStableFrontendContract(t *testing.T) {
	...
	if result.Options.Target != "ES2023" || result.Options.Module != "ESNext" || !result.Options.Strict {
		t.Fatalf("compiler options = %+v, want normalized strict ES2023/ESNext options", result.Options)
	}
