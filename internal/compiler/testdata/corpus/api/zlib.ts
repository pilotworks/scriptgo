...
deflate("hello", (err: unknown, res: Uint8Array) => {
    console.log("zlib_deflate_cb: " + (res.length >= 0));
});

// @api: zlib.deflateSync
// @expect: zlib_deflateSync: true
console.log("zlib_deflateSync: " + (deflateSync("hello").length >= 0));

// @api: zlib.deflateRaw
// @expect: zlib_deflateRaw_cb: true
deflateRaw("hello", (err: unknown, res: Uint8Array) => {
...
gzip("hello", (err: unknown, res: Uint8Array) => {
    console.log("zlib_gzip_cb: " + (res.length >= 0));
});

// @api: zlib.gzipSync
// @expect: zlib_gzipSync: true
console.log("zlib_gzipSync: " + (gzipSync("hello").length >= 0));

// @api: zlib.gunzip
// @expect: zlib_gunzip_cb: true
gunzip("hello", (err: unknown, res: Uint8Array) => {
    console.log("zlib_gunzip_cb: " + (res.length >= 0));
});

// @api: zlib.gunzipSync
// @expect: zlib_gunzipSync: true
console.log("zlib_gunzipSync: " + (gunzipSync("hello").length >= 0));

// @api: zlib.inflate
// @expect: zlib_inflate_cb: true
inflate("hello", (err: unknown, res: Uint8Array) => {
    console.log("zlib_inflate_cb: " + (res.length >= 0));
});

// @api: zlib.inflateSync
// @expect: zlib_inflateSync: true
console.log("zlib_inflateSync: " + (inflateSync("hello").length >= 0));

// @api: zlib.inflateRaw
// @expect: zlib_inflateRaw_cb: true
inflateRaw("hello", (err: unknown, res: Uint8Array) => {
    console.log("zlib_inflateRaw_cb: " + (res.length >= 0));
});

// @api: zlib.inflateRawSync
// @expect: zlib_inflateRawSync: true
console.log("zlib_inflateRawSync: " + (inflateRawSync("hello").length >= 0));

// @api: zlib.unzip
// @expect: zlib_unzip_cb: true
unzip("hello", (err: unknown, res: Uint8Array) => {
    console.log("zlib_unzip_cb: " + (res.length >= 0));
});

// @api: zlib.unzipSync
// @expect: zlib_unzipSync: true
console.log("zlib_unzipSync: " + (unzipSync("hello").length >= 0));

// @api: zlib.brotliCompress
// @expect: zlib_brotliCompress_cb: true
brotliCompress("hello", (err: unknown, res: Uint8Array) => {
    console.log("zlib_brotliCompress_cb: " + (res.length >= 0));
});

// @api: zlib.brotliCompressSync
// @expect: zlib_brotliCompressSync: true
console.log("zlib_brotliCompressSync: " + (brotliCompressSync("hello").length >= 0));

// @api: zlib.brotliDecompress
// @expect: zlib_brotliDecompress_cb: true
brotliDecompress("hello", (err: unknown, res: Uint8Array) => {
    console.log("zlib_brotliDecompress_cb: " + (res.length >= 0));
});

// @api: zlib.brotliDecompressSync
// @expect: zlib_brotliDecompressSync: true
console.log("zlib_brotliDecompressSync: " + (brotliDecompressSync("hello").length >= 0));

// @api: zlib.zstdCompress
// @expect: zlib_zstdCompress_cb: true
zstdCompress("hello", (err: unknown, res: Uint8Array) => {
    console.log("zlib_zstdCompress_cb: " + (res.length >= 0));
});

// @api: zlib.zstdCompressSync
// @expect: zlib_zstdCompressSync: true
console.log("zlib_zstdCompressSync: " + (zstdCompressSync("hello").length >= 0));

// @api: zlib.zstdDecompress
// @expect: zlib_zstdDecompress_cb: true
zstdDecompress("hello", (err: unknown, res: Uint8Array) => {
    console.log("zlib_zstdDecompress_cb: " + (res.length >= 0));
});

// @api: zlib.zstdDecompressSync
// @expect: zlib_zstdDecompressSync: true
console.log("zlib_zstdDecompressSync: " + (zstdDecompressSync("hello").length >= 0));
