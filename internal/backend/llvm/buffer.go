...
func (e *functionEmitter) emitBufferIntrinsic(out *strings.Builder, instruction ir.Instruction) error {
	...
	case "__buffer.from_arraybuffer":
		slot := instruction.Result + ".slot"
		status := fmt.Sprintf("runtime.status.%d", e.runtimeStatus)
		e.runtimeStatus++
		fmt.Fprintf(out, "  %%%s = alloca ptr\n", slot)
		fmt.Fprintf(out, "  %%%s = call i32 @scriptgo_buffer_from_arraybuffer(ptr %%%s, ptr %%%s)\n",
	...
	default:
		return fmt.Errorf("unhandled buffer operation: %s", instruction.Callee)
	}
}
