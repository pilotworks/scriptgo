# Assert Implementation Checklist

> **Category**: `CategoryNodeModule`  
> **Import Path**: `node:assert`  
> **Specification Reference**: [Node.js 22 LTS Assert Documentation](https://nodejs.org/docs/latest-v22.x/api/assert.html)  
> **Type Definition Source**: [@types/node/assert.d.ts](https://github.com/DefinitelyTyped/DefinitelyTyped/tree/master/types/node)  
> **Gate Oracle**: Node.js 22 LTS test suite (test/parallel/test-assert-*.js)
...
- **Scope & Exposure**: Module-scoped symbols imported explicitly via `node:assert`.
...
Corpus test cases for `assert` are organized per API under `internal/compiler/testdata/corpus/assert/<api_name>/<test_case>/` and verify identical lowering semantics across surfaces.
...
- [ ] **Step 6: Corpus Test Directory**: Create test subfolder under `internal/compiler/testdata/corpus/assert/<api_name>/<test_case>/` with `main.ts` and `run.expected`.
