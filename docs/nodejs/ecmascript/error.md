...
> **Specification Reference**: [TC39 ECMA-262 Error Specification](https://tc39.es/ecma262/#sec-error-objects)  
...
Corpus test cases for `error` are organized per API under `internal/compiler/testdata/corpus/error/<api_name>/<test_case>/` and verify identical lowering semantics across surfaces.
...
- [ ] **Step 6: Corpus Test Directory**: Create test subfolder under `internal/compiler/testdata/corpus/error/<api_name>/<test_case>/` with `main.ts` and `run.expected`.
