...
> **Specification Reference**: [TC39 ECMA-262 Map Specification](https://tc39.es/ecma262/#sec-map-objects)  
...
Corpus test cases for `map` are organized per API under `internal/compiler/testdata/corpus/map/<api_name>/<test_case>/` and verify identical lowering semantics across surfaces.
...
- [ ] **Step 6: Corpus Test Directory**: Create test subfolder under `internal/compiler/testdata/corpus/map/<api_name>/<test_case>/` with `main.ts` and `run.expected`.
