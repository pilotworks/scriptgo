...
| **Dynamic**     | Opt-in with `--dynamic` | Execute the supported local JavaScript island through embedded QuickJS-ng, including native closure callbacks at the boxed boundary. | Report an actionable Dynamic boundary diagnostic when the site is outside the v1 island.                |
