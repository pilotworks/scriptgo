...
| **Buffer & Binary Operations (`node:buffer`)** | ✅ Full (91 / 91 APIs) | Complete 100% Node.js parity: `Buffer.from(ArrayBuffer)` and array/typed-array inputs, byte-backed `Blob` construction / slicing / `arrayBuffer()` / `text()`, `File`, `SlowBuffer`, `atob`, `btoa`, `isAscii`, `isUtf8`, `resolveObjectURL`, `transcode`, all integer/float/bigint BE/LE read/write methods, `swap16/32/64`, `includes`, `lastIndexOf`, `write`, `subarray`, `slice`, and constants (`MAX_LENGTH`, `MAX_STRING_LENGTH`, `kMaxLength`, `kStringMaxLength`, `INSPECT_MAX_BYTES`). |
| **Web Streams API (`node:webstreams` / `node:stream/web`)** | ✅ Full (85 / 85 APIs) | Complete 100% Node.js parity: `ReadableStream`, `ReadableStreamDefaultReader`, `ReadableStreamBYOBReader`, `ReadableStreamDefaultController`, `ReadableByteStreamController`, `ReadableStreamBYOBRequest`, `WritableStream`, `WritableStreamDefaultWriter`, `WritableStreamDefaultController`, `TransformStream`, `TransformStreamDefaultController`, `ByteLengthQueuingStrategy`, `CountQueuingStrategy`, `TextEncoderStream`, `TextDecoderStream`, `CompressionStream`, `DecompressionStream`, `from`, `arrayBuffer`, `blob`, `buffer`, `json`, `text`; verified callback dispatch through optional source/sink fields and `null` stream state preservation. |
