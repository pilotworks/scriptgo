...
| **Readline CLI (`node:readline` & `node:readline/promises`)** | ✅ Core APIs | Genuine C-backed TTY readline runtime: `Interface`, `createInterface`, `clearLine`, `clearScreenDown`, `cursorTo`, `moveCursor`, `emitKeypressEvents`, promise-based `node:readline/promises` (`Interface`, `createInterface`), prompt, line queue, question callbacks, and async iterators. |
...
| **Terminal TTY (`node:tty`)** | ✅ Core APIs | Real POSIX/Windows C runtime TTY support: `isatty`, `ReadStream`, `WriteStream`, `getWindowSize`, `setRawMode`, `isTTY`, `columns`, `rows`, and graceful non-interactive fallback. |
