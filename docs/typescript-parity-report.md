...
| **`node:test` / `test`** | `test`, `it`, `describe`, `suite`, `before`, `after`, `beforeEach`, `afterEach`, `mock`, `TestContext`, `SuiteContext`, subtests, TAP/spec reporting | ✅ 100% matches Node.js test runner specification |
...
| **Events (`node:events`)** | ✅ Complete (69 / 69 official APIs; 100.0%) | Complete 100% Node.js parity: `EventEmitter` (with `'newListener'`, `'removeListener'`, unhandled error throwing, `errorMonitor`, `captureRejections`), `EventEmitterAsyncResource` (`emitDestroy`, `asyncId`, `triggerAsyncId`, `asyncResource`), `NodeEventTarget`, `Event`, `CustomEvent`, `EventTarget`, `getEventListeners`, `getMaxListeners`, `setMaxListeners`, `listenerCount`, `once`, `on`, `addAbortListener`, `defaultMaxListeners`, `errorMonitor`, `captureRejectionSymbol`. |
...
| **Terminal TTY (`node:tty`)** | ✅ Full (16 / 16 APIs; 100.0%) | Complete 100% Node.js parity: `isatty`, `ReadStream` (`fd`, `isRaw`, `isTTY`, `setRawMode`), `WriteStream` (`fd`, `isTTY`, `columns`, `rows`, `clearLine`, `clearScreenDown`, `cursorTo`, `moveCursor`, `getColorDepth`, `hasColors`, `getWindowSize`), and graceful non-interactive fallback. |
| **Path Utilities (`node:path`)** | ✅ Full (15 / 15 APIs) | Complete 100% Node.js parity: `basename`, `dirname`, `extname`, `format`, `isAbsolute`, `join`, `normalize`, `parse`, `relative`, `resolve`, `toNamespacedPath`, `delimiter`, `sep`, `posix`, `win32`. Naive `matchesGlob` stub removed. |
| **Streams (`node:stream`)** | ✅ Complete (15 / 15 official APIs; 100.0%) | Complete 100% Node.js parity: `Readable`, `Writable`, `Duplex`, `Transform`, `PassThrough`, `pipeline`, `finished`, `compose`, `from`, `fromWeb`, `toWeb`, `getDefaultHighWaterMark`, `setDefaultHighWaterMark`, `[Symbol.asyncIterator]`, `node:stream/promises` (`pipeline`, `finished`), and `node:stream/consumers` (`buffer`, `text`, `json`, `arrayBuffer`, `blob`). |
...
7. **Compiler-Proven Temporary Object Regions**: At `-O2` and higher, a target-independent IR pass identifies a non-escaping object graph builder followed only by a read-only visitor inside a scope. The runtime allocates that graph outside tracing GC and resets the whole region after the visitor completes. Escaping values, writes, dynamic calls, and external calls disqualify a region.
