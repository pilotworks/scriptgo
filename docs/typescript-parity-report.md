...
| **Web Cryptography API (`node:webcrypto`)** | ✅ Core APIs | Real HMAC signing/verification (`subtle.sign`, `subtle.verify`), SHA-256 digests (`subtle.digest`), PBKDF2 bit derivation (`subtle.deriveBits`, `subtle.deriveKey`), key generation (`subtle.generateKey`), raw import/export (`subtle.importKey`, `subtle.exportKey`), `CryptoKey`, `CryptoKeyPair`, `Algorithm`, `KeyAlgorithm`, `randomUUID`, `getRandomValues`. Stub methods (`encrypt`, `decrypt`, `wrapKey`, `unwrapKey`) removed. |
...
| **Utilities (`node:util`)** | ✅ Core APIs | `format`, `formatWithOptions`, `inspect`, `isDeepStrictEqual` (real recursive assertion-backed equality), `types.*` (genuine type predicates), all `is*` type predicates, `deprecate`, `_extend`, `toUSVString`, `stripVTControlCharacters`, `styleText`, `getSystemErrorName/Map/Message`, `getCallSites`, `setTraceSigInt`, `parseArgs`, `parseEnv`, `promisify`, `callbackify`, `debug`, `debuglog`, `diff`, `aborted`, `transferableAbort*`, `log`, `MIMEType`, `MIMEParams`, `TextDecoder`, and `TextEncoder`. Unsupported `isWeakMap` and `isWeakSet` removed. |
| **Networking & Sockets (`node:net`)** | ✅ Core APIs | Real POSIX/BSD socket implementation: `Server`, `Socket`, `SocketAddress`, `connect`, `createConnection`, `createServer`, `isIP`, `isIPv4`, `isIPv6`, `[Symbol.asyncDispose]`. Dummy string matching `BlockList` and `autoSelectFamily` hooks removed. |
...
| **Diagnostics Channel (`node:diagnostics_channel`)** | ✅ Core APIs | `channel`, `hasSubscribers`, `Channel`, `TracingChannel`, `traceSync`, `tracePromise`, `traceCallback`. |
...
| **DNS Resolution (`node:dns`)** | ✅ Core APIs | C runtime getaddrinfo / resolver bindings: `lookup`, `lookupService`, `resolve`, `resolve4`, `resolve6`, `resolveCname`, `resolveMx`, `resolveNs`, `resolveTxt`, `resolveSrv`, `resolvePtr`, `resolveNaptr`, `resolveSoa`, `resolveCaa`, `reverse`, `getServers`, `setServers`, `setDefaultResultOrder`, `getDefaultResultOrder`, `Resolver`, `promises.*`. |
...
| **UDP / Datagram Sockets (`node:dgram`)** | ✅ Core APIs | Real BSD socket UDP implementation: `Socket` (bind, send, close, address, getRecvBufferSize, getSendBufferSize, setBroadcast, setMulticastLoopback, setMulticastTTL, setTTL, addMembership, dropMembership), `createSocket`, `[Symbol.asyncDispose]`. Dummy queue size/ref stubs removed. |
...
| **Timers Scheduling (`node:timers`)** | ✅ Core APIs (20 / 21 APIs) | `setTimeout`, `clearTimeout`, `setInterval`, `clearInterval`, `setImmediate`, `clearImmediate`, `Immediate` (`hasRef`, `ref`, `unref`, `[Symbol.dispose]`), `Timeout` (`hasRef`, `ref`, `unref`, `close`, `[Symbol.toPrimitive]`, `[Symbol.dispose]`), `promises` (`setTimeout`, `setImmediate`, `setInterval`, `scheduler.wait`, `scheduler.yield`). Dummy no-op `Timeout.refresh` removed. |
...
| **Streams (`node:stream`)** | ✅ Full (15 / 15 APIs) | Complete 100% Node.js parity: `Readable`, `Writable`, `Duplex`, `Transform`, `PassThrough`, `pipeline`, `finished`, `compose`, `getDefaultHighWaterMark`, `setDefaultHighWaterMark`, `promises` (`pipeline`, `finished`), and `node:stream/consumers` (`buffer`, `text`, `json`, `arrayBuffer`, `blob`). |
